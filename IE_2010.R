...
base_hijos <- read_dta("Bases de datos/MCS 2010/base_hijos_mf.dta")
base_madre <- read_dta("Bases de datos/MCS 2010/base_madre_mf.dta")
base_padre <- read_dta("Bases de datos/MCS 2010/base_padre_mf.dta")
...
datos2010 <- read_excel("Bases de datos/MCS 2010/datos2010.xlsx")

# Aqui aniado variables de interes por entidad federativa para todas las observaciones para correr el modelo
mergep <- left_join(mergep, datos2010, by = "codigo")
mergem <- left_join(mergem, datos2010, by = "codigo")
...
model_entidad  <- function(df) {
  lm(IEhij ~ IEpad + `Pobreza 2010`+ log10(EscolProm2010), data = df)
}
...
coeff_p %>% 
  ...
  labs(title = "Desigualdad de oportunidades por entidad federativa",
       ...
       x = "Entidad federativa",
       y = "Estimacion",
       caption = "Fuente: Elaborado por Sergio Parra con datos de la ENIGH 2010.") +
  theme(axis.text.x = element_text(angle = 90, 
                                   hjust = 1,# just "horizontal" de texto 
                                   vjust = 0.5))
...
model_entidad_m  <- function(dfm) {
  lm(IEhij ~ IEmad + `Pobreza 2010` + log10(EscolProm2010), data = dfm)
}

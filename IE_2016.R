...
#ENIGH 2016 ----
base_hijos <- read_dta("Bases de datos/MCS 2016/base_hijos_mf.dta")
base_madre <- read_dta("Bases de datos/MCS 2016/base_madre_mf.dta")
base_padre <- read_dta("Bases de datos/MCS 2016/base_padre_mf.dta")
...
datos2016 <- read_excel("Bases de datos/MCS 2016/datos2016.xlsx")

# Aqui aniado variables de interes por entidad federativa para todas las observaciones para correr el modelo
mergep <- left_join(mergep, datos2016, by = "codigo")
mergem <- left_join(mergem, datos2016, by = "codigo")
...
model_entidad  <- function(df) {
  lm(IEhij ~ IEpad + `Pobreza 2016`+ log10(EscolProm2016), data = df)
}
...
model_entidad_m  <- function(dfm) {
  lm(IEhij ~ IEmad + `Pobreza 2016` + log10(EscolProm2016), data = dfm)
}
...
PIBEstados <- read_excel("Bases de datos/PIBEstados.xlsx") 
...
logprom_17 <- PIBEstados %>% 
  ...
  mutate(valor_lead_1 = lead(valor),
         log_promedio = ((log10(valor_lead_1) - log10(valor))*100) )


#Nos quedamos solo con el anio 2016 y el crecimiento del 2017 
crec2017<- logprom_17 %>% 
  filter(Ano == 2016) %>% 
  select(Ano, entidad, log_promedio, valor) %>% 
  mutate(EDO = 1:32)

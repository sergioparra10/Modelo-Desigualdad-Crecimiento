#######################################################
### Tesis: Desigualdad de oportunidades y crecimiento economico en Mexico ENIGH 2012
#######################################################
...
poblacion12 <- read.dta("Desktop/Tesis/Bases de datos/MCS 2012/pobla12.dta", convert.dates = TRUE, convert.factors = TRUE)

#### Indice de Educacion ----

poblacion12 <- poblacion12 %>% 
  filter(!(parentesco >400 & parentesco<500)) %>% 
  filter(!(parentesco>700 & parentesco<=715))

table(poblacion12$parentesco)

#Asistencia Escolar 

poblacion12$asis_esc<-as.numeric(poblacion12$asis_esc)
#Definicion
...
mean(poblacion12$Esc, na.rm = TRUE)

table(poblacion12$Esc)
...
table(poblacion12$IE)
mean(poblacion12$IE, na.rm = TRUE)

#Asignamos a los menores de 6 el indice promedio del hogar**

poblacion12[order(poblacion10$folioviv, poblacion10$foliohog),]

poblacion12<- poblacion12 %>% 
  mutate(folio = str_c(folioviv, foliohog, collapse = NULL))

poblacion12[order(poblacion12$folio, poblacion12$folioviv),]
...
print(poblacion12$MediaIE)
sum(is.na(poblacion12$MediaIE)) 

poblacion12 <- poblacion12 %>% 
  mutate(IE = replace(IE, edad<6, MediaIE)) 
sum(is.na(poblacion12$IE)) 
...
poblacion12 <- select (poblacion12,-c(sexo))
poblacion12 <- poblacion12 %>% rename(sexo = sex)

# Base de hijos

poblacion12_h <- poblacion12

poblacion12_h <- poblacion12_h %>% 
...
poblacion12_p <- poblacion12
poblacion12_p<- subset(poblacion12_p, sexo == 0 )

poblacion12_p <- poblacion12_p %>% 
...
colnames(poblacion12_p)


#Base de madres 

poblacion12_m <- poblacion12

poblacion12_m<- subset(poblacion12_m, sexo == 1 )

###Nota: la base en r ya me lo toma como character, no necesito hacer transformacion

poblacion12_m <- poblacion12_m %>% 
  mutate(leng = str_length(numren))
...
colnames(poblacion12_m)

#### Merge ----

#mergep

mergep <- merge(poblacion12_h, poblacion12_p, by =c("folioviv","foliohog","IDp"),suffixes = c(".h",".p"))

#mergem 

mergem <- merge(poblacion12_h, poblacion12_m, by =c("folioviv","foliohog","IDm"),suffixes = c(".h",".m"))
...
summary(poblacion12$Esc, na.rm = TRUE)
...
#Promedio crecimiento 2012-2017
logprom_12_17 <- PIBEstados %>% 
  gather(key = entidad,
         value = valor, 
         -Ano) %>% 
  ...
  mutate(valor_lead_1 = lead(valor),
         ...
         log_promedio = (((log10(valor_lead_5) - log10(valor_lead_1))/5)*100) )


#Nos quedamos solo con 2010
crec2012<- logprom_12_17 %>% 
  filter(Ano == 2012) %>% 
...
io_p<- merge(crec2012, coeff_p, by = "EDO")
...
io_p_r <- merge(crec2012, rsquared_p, by = "EDO")
...
io_m<- merge(crec2012, coeff_m, by = "EDO")
...
io_m_r <- merge(crec2012, rsquared_m, by = "EDO")

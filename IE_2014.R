...
base_hijos <- read_dta("Bases de datos/MCS 2014/base_hijos_mf.dta")
base_madre <- read_dta("Bases de datos/MCS 2014/base_madre_mf.dta")
base_padre <- read_dta("Bases de datos/MCS 2014/base_padre_mf.dta")
...
datos2014 <- read_excel("Bases de datos/MCS 2014/datos2014.xlsx")

# Aqui aniado variables de interes por entidad federativa para todas las observaciones para correr el modelo
mergep <- left_join(mergep, datos2014, by = "codigo")
mergem <- left_join(mergem, datos2014, by = "codigo")
...
model_entidad  <- function(df) {
  lm(IEhij ~ IEpad + `Pobreza 2014`+ log10(EscolProm2014), data = df)
}
...
model_entidad_m  <- function(dfm) {
  lm(IEhij ~ IEmad + `Pobreza 2014` + log10(EscolProm2014), data = dfm)
}

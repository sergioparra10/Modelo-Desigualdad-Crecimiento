...
#ENIGH 2016 ----
poblacion_or <- read_dta("Desktop/Tesis/Bases de datos/MCS 2016/poblacion.dta")

poblacion <- poblacion_or
#### Indice de Educacion ----
...
table(poblacion$parentesco)

#Asistencia Escolar 

poblacion$asis_esc<-as.numeric(poblacion$asis_esc)
...
#Nomralizamos la edad para tener educacion esperada. 
poblacion <- poblacion %>% 
  mutate(
    norm = case_when(
      edad==6  ~ 1 ,
...
#Falta condicion          gen Esc=. if nivelaprob<1
#                       mutate(Esc = na_if(Esc, nivelaprob <1))
...
table(poblacion$esc_esp)
names(poblacion)

#Generamos la tasa de esc dividiendo la esccolaridad entre la norma**

##ERROR NO ME ESTA GENERANDO DE MANERA CORRECTA LOS INDICES, FALTAN MUCHOS CEROS VER CONDICION FALTANTE 
#mutate(IE = replace(IE, IE != is.na(IE), 1)) #Falta ver esta condicion en do file 
...
mean(poblacion$IE, na.rm = TRUE)
    
#Asignamos a los menores de 6 el indice promedio del hogar**

poblacion[order(poblacion$folioviv, poblacion$foliohog),]

poblacion <- poblacion %>% 
  mutate(folio = str_c(folioviv, foliohog, collapse = NULL))

poblacion[order(poblacion$folio, poblacion$folioviv),]

poblacion <- poblacion %>% 
  transform %>% 
  group_by(folio) %>% 
  mutate(MediaIE = mean(IE, na.rm = TRUE))

print(poblacion$MediaIE)
sum(is.na(poblacion$MediaIE)) 
...
poblacion<- select (poblacion,-c(sexo))
poblacion <- poblacion %>% rename(sexo = sex)
colnames(poblacion)


# Base de hijos

poblacion_h <- poblacion

poblacion_h <- poblacion_h %>% 
  filter((madre_hog == 1 | padre_hog == 1))

poblacion_h <- poblacion_h %>% 
  mutate(IDp = str_c(folioviv, foliohog, padre_id,collapse = NULL))

poblacion_h <- poblacion_h %>% 
  mutate(IDm = str_c(folioviv, foliohog, madre_id,collapse = NULL))


#Base de padres: nos quedamos con los hombres 

poblacion_p <- poblacion
poblacion_p<- subset(poblacion_p, sexo == 0 )
...
###NO SE CUAL COMANDO ES EL CORRECTO 
#poblacion <- poblacion %>% 
#  mutate(numren = replace(numren, leng == 1, 0)) 
...
mergep <- merge(poblacion_h, poblacion_p, by =c("folioviv","foliohog","IDp"),suffixes = c(".h",".p"))

#mergem 

mergem <- merge(poblacion_h, poblacion_m, by =c("folioviv","foliohog","IDm"),suffixes = c(".h",".m"))
...
ggcoef(resul_edos, exponentiate = FALSE, exclude_intercept = TRUE)+
  coord_flip()

#Aqui ya me los da como los quiero pero de manera incorrecta solo que sumados con un uno y el coeficiente modificado 
...
coeff_m %>% 
  ...
  expand_limits(x = 0, y = 0.1) +
  labs(title = "Desigualdad de oportunidades por entidad federativa",
       subtitle = "Esta grafica muestra la persistencia de la desigualdad de oportunidades (madres)",
       x = "Entidad federativa",
       y = "Estimacion",
       caption = "Fuente: Elaborado por Sergio Parra con datos de la ENIGH.") +
  theme(axis.text.x = element_text(angle = 90, 
...
linearMod_hm <- lm(IE.h ~ IE.m , data=mergem)
print(linearMod_hm)
stargazer(linearMod_hm)

#Ahora para los padres y todos los hijos
linearMod_hp <- lm(IE.h ~ IE.p , data=mergep)
print(linearMod_hp)
stargazer(linearMod_hp)

#Hagamos una regresion a nivel nacional con un termino interactuado, lanza resultados significativos
...
summary(poblacion$Esc, na.rm = TRUE)

#Escolaridad frecuencia acumulada 
ggplot(poblacion, aes(Esc)) + stat_ecdf() +
  labs(title = "Escolaridad para mayores de 25 a??os",
...
ggplot(poblacion, aes(x=IE, color = sexo)) + 
  geom_density(color="darkblue", fill="lightblue") +
  labs(title = "Diagrama de densidad",
       subtitle = "Density plot de indice educativo",
       x = "indice Educativo",
       y = "Densidad",
       caption = "Fuente: Elaborado por Sergio Parra con datos del INEGI.")
...
#Promedio crecimiento 2016-2017
logprom_17 <- PIBEstados %>% 
  gather(key = entidad,
         value = valor, 
         -Ano) %>% 
  ...
  mutate(valor_lead_1 = lead(valor),
         log_promedio = (((log10(valor_lead_1) - log10(valor))/2)*100) )


#Nos quedamos solo con el anio 2016 y el crecimiento del 2017 
crec2017<- logprom_17 %>% 
  filter(Ano == 2016) %>% 
...
io_p<- merge(crec2017, coeff_p, by = "EDO")
...
io_p_r <- merge(crec2017, rsquared_p, by = "EDO")
...
io_m<- merge(crec2017, coeff_m, by = "EDO")
...
io_m_r <- merge(crec2017, rsquared_m, by = "EDO")

#######################################################
### Tesis: Desigualdad de oportunidades y crecimiento economico en Mexico ENIGH 2014 
#######################################################
...
poblacion14 <- read_dta("Desktop/Tesis/Bases de datos/MCS 2014/mcs_poblacion_2014_concil_2010.dta")

#### Indice de Educacion ----

poblacion14$parentesco<- as.numeric(poblacion14$parentesco)

poblacion14 <- poblacion14 %>% 
  filter(!(parentesco >400 & parentesco<500)) %>% 
  filter(!(parentesco>700 & parentesco<=715))

table(poblacion14$parentesco)

#Asistencia Escolar 

poblacion14$asis_esc<-as.numeric(poblacion14$asis_esc)
...
mean(poblacion14$Esc, na.rm = TRUE)

table(poblacion14$Esc)


### ERROR. No genera los ceros que deberia generar : 
...
table(poblacion14$IE)

mean(poblacion14$IE, na.rm = TRUE)

#Asignamos a los menores de 6 el indice promedio del hogar**

poblacion14[order(poblacion14$folioviv, poblacion14$foliohog),]

poblacion14 <- poblacion14 %>% 
  mutate(folio = str_c(folioviv, foliohog, collapse = NULL))

poblacion14[order(poblacion14$folio, poblacion14$folioviv),]
...
poblacion14<- select (poblacion14,-c(sexo))
poblacion14 <- poblacion14 %>% rename(sexo = sex)
colnames(poblacion14)


# Base de hijos

poblacion14_h <- poblacion14

poblacion14_h <- poblacion14_h %>% 
...
poblacion14_p <- poblacion14
poblacion14_p<- subset(poblacion14_p, sexo == 0 )

poblacion14_p <- poblacion14_p %>% 
...
colnames(poblacion14_p)


#Base de madres 

poblacion14_m <- poblacion14

poblacion14_m<- subset(poblacion14_m, sexo == 1 )

###Nota: la base en r ya me lo toma como character, no necesito hacer transformacion

poblacion14_m <- poblacion14_m %>% 
  mutate(leng = str_length(numren))
...
mergep <- merge(poblacion14_h, poblacion14_p, by =c("folioviv","foliohog","IDp"),suffixes = c(".h",".p"))

#mergem 

mergem <- merge(poblacion14_h, poblacion14_m, by =c("folioviv","foliohog","IDm"),suffixes = c(".h",".m"))
...
summary(poblacion14$Esc, na.rm = TRUE)
...
#Promedio crecimiento 2014-2017
logprom_14_17 <- PIBEstados %>% 
  gather(key = entidad,
         value = valor, 
         -Ano) %>% 
  ...
  mutate(valor_lead_1 = lead(valor),
         ...
         log_promedio = (((log10(valor_lead_3) - log10(valor_lead_1))/3)*100) )


#Nos quedamos solo con 2014
crec2014<- logprom_14_17 %>% 
  filter(Ano == 2014) %>% 
...
io_p<- merge(crec2014, coeff_p, by = "EDO")
...
io_p_r <- merge(crec2014, rsquared_p, by = "EDO")
...
io_m<- merge(crec2014, coeff_m, by = "EDO")
...
io_m_r <- merge(crec2014, rsquared_m, by = "EDO")
